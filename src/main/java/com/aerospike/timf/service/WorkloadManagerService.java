...
public class WorkloadManagerService {
    ...
    public synchronized <C> void createDatabaseInstance(DatabaseParameters parameters, DatabaseFunctions<C> databaseFunctions) {
        String name = parameters.getName();
        if (databaseInstances.containsKey(name)) {
            throw new IllegalArgumentException("database " + name + " already exists");
        }
        C databaseConnection = databaseFunctions.connectInstance(parameters.getConfigParams());
        WorkloadExecutor<C> executionDetails = new WorkloadExecutor<C>(name, databaseConnection, databaseFunctions, timingService);
